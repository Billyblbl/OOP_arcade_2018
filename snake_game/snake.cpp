...
{
    float end = _snake.size();

    if ((_snake[0].x == _food.x) && (_snake[0].y == _food.y)) {
        Snake::place_food();
        if (_snake[end].x == (_snake[end - 1].x - 1))
            _snake.push_back({_snake[end].x + 1, _snake[end].y});
        if (_snake[end].x == (_snake[end - 1].x + 1))
            _snake.push_back({_snake[end].x - 1, _snake[end].y});
        if (_snake[end].x == (_snake[end - 1].y - 1))
            _snake.push_back({_snake[end].y - 1, _snake[end].y});
        if (_snake[end].x == (_snake[end + 1].y - 1))
            _snake.push_back({_snake[end].y + 1, _snake[end].y});
    }
}
...
void Snake::display()
{
    IDisplayable *snake_head = _screen->createDisplayable("snake_head.txt");
    std::vector<IDisplayable *> snake_tail;
    IDisplayable *fruit = _screen->createDisplayable("fruit.txt");

    for (int i = 0; i < _snake.size() - 1; i++)
        snake_tail.push_back = _screen->createDisplayable("snake_tail.txt");
